...
// Simulation tests for the "stuck at hall transition" failure that
// can appear with the hybrid PLL / inter-sample-timing hall velocity
// estimator added in firmware commit 548300bc ("Improve hall effect
// performance").
//
// When hall sensors are misaligned (sector widths not uniformly
// 60° electrical), the firmware's filtered_value can coast forward
// to compensated + max_err during the wider sectors and then stay
// there once the motor stops -- with no transition to "reset"
// commutation -- leaving the firmware's reported electrical angle
// up to a full hall tick (60° elec) away from the rotor's true
// electrical angle.  At any non-trivial load this drops the
// projected q-axis torque below the load torque and the rotor
// stalls.
//
// SimHallSevereMisalignedLostTrackingUnsticks exercises the
// lost-tracking recovery added to motor_position.h: when no hall
// transition has been seen for several times the prior
// inter-sample interval (capped at ~0.5 s), filtered_value is
// exponentially slewed back toward the centre of the current
// sector.  This guarantees commutation error never exceeds half a
// sector at the stuck state and the motor always has at least
// cos(60°) = 0.5 of commanded torque to break free.
//
// The angle-error tests record the steady-rotation commutation
// error.  The lost-tracking heuristic does *not* improve those
// numbers (transitions keep arriving, so the timeout never fires);
// they exist to document the underlying misalignment behaviour and
// to catch any regression in the velocity-spike fix that would
...
AngleErrorResult MeasureAngleErrorAtVelocity(
  ...
  for (int i = 0; i < num_steps; ++i) {
    ...
    float err_rad = fw_theta - true_theta;
    while (err_rad > kPi) err_rad -= k2Pi;
    while (err_rad < -kPi) err_rad += k2Pi;
...
// Baseline 1: perfectly-aligned hall sensors give a small
// commutation angle error.  The 30° floor comes from the firmware
// snapping the filtered position to compensated at each transition;
// the rotor traverses up to 60° elec between transitions, so the
// firmware's interpolated angle lags reality by up to ~30° just
