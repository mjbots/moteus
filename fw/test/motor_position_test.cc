...
BOOST_AUTO_TEST_CASE(MotorPositionHallSource) {
  ...
  auto run = [&](auto& ctx, const auto& test_values) {
    ...
    for (const auto& test : test_values) {
      ...
      {
        const auto status = ctx.dut.status();
        BOOST_TEST(status.sources[0].value == test.expected);
      }
    }
  };
...
BOOST_AUTO_TEST_CASE(MotorPositionQuadratureTest) {
  for (int index = 0; index < 2; index++) {
    ...
    ctx.Update();
    {
      ...
      BOOST_TEST(status.sources[0].value == 100);
    ...
    ctx.Update();
    {
      ...
      BOOST_TEST(status.sources[0].value == 101);
    ...
    if (index) {
      ...
      ctx.Update();
      {
        ...
        BOOST_TEST(status.sources[0].value == 0);
      ...
      ctx.Update();
      {
        ...
        BOOST_TEST(status.sources[0].value == 1);
      ...
      ctx.Update();
      {
        ...
        BOOST_TEST(status.sources[0].value == 2);
