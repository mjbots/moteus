...
class BldcServoPosition {
 public:
  ...
  static void DoVelocityAndAccelLimits(
    ...
    const float v_abs = std::abs(v_frame);
    ...
    // Check for mid-step switch from accel to decel.
    // This is the key to matching analytical trajectories exactly.
    float t_switch = period_s;
    if (moving_towards && !std::isfinite(data->velocity_limit)) {
      t_switch = ComputeSwitchTime(v_abs, dx_abs, a, period_s);
    }
    ...
    } else if (moving_towards && t_switch == 0.0f) {
      ...
      *status->control_velocity += status->control_acceleration * period_s;
      ...
      if (std::isfinite(data->velocity_limit) && std::abs(v0) >= data->velocity_limit) {
        ...
        *status->control_velocity += status->control_acceleration * period_s;
        ...
        if (std::isfinite(data->velocity_limit) && std::abs(v1) > data->velocity_limit) {
      ...
      // Moving away from target - decelerate toward it
      status->control_acceleration = std::copysign(a, -v_frame);
      *status->control_velocity += status->control_acceleration * period_s;
      v1 = *status->control_velocity;
      position_step = (v0 + v1) * 0.5f * period_s;
    ...
    // Fixed threshold: 1e-5 rev ≈ 0.004 degrees
    constexpr float kPositionThreshold = 1e-5f;
    const float v1_frame = v1 - vf;
    const bool velocity_near_target = std::abs(v1_frame) < (a * period_s);
    const bool position_near_target = std::abs(dx_after_step) < kPositionThreshold;

    // Complete if position crossed target while decelerating with low velocity.
    const bool crossed_target = (dx > 0.0f && dx_after_step <= 0.0f) ||
                                 (dx < 0.0f && dx_after_step >= 0.0f);
    const bool decelerating = status->control_acceleration * v_frame < 0.0f;

    if ((crossed_target && decelerating && velocity_near_target) ||
        (position_near_target && velocity_near_target)) {
      data->position = std::numeric_limits<float>::quiet_NaN();
      data->position_relative_raw.reset();
      status->control_acceleration = 0.0f;
