...
public:
  ...
  // Command IDs (low 5 bits of CAN ID).
  enum CmdId : uint8_t {
  ...
  // Sentinel values for expected dlc.
  static constexpr int8_t kDlcAny       = -1;  // any length allowed
  static constexpr int8_t kDlcNotUsed   = -2;  // command not accepted from host
  ...
  bool HandleFrame(uint32_t can_id, int dlc, const char *data) {
    ...
    if (node_id != multiplex_protocol_->config()->id) { return false; }

    debug_led_canfd = 1;

    if (cmd_id >= CAN_CMD_COUNT) { return false; }

    const CmdEntry& entry = kCommandTable[cmd_id];
    if (entry.handler == nullptr) { return false; }

    if (entry.expected_dlc == kDlcNotUsed) { return false; }
    if (entry.expected_dlc >= 0 && dlc != entry.expected_dlc) { return false; }

    const uint8_t* data_bytes = reinterpret_cast<const uint8_t*>(data);
    const bool ok = (this->*(entry.handler))(dlc, data_bytes);
...
private:
  bool HandleMotorDisable     (int dlc, const uint8_t* data) { return true; }
  bool HandleMotorEnable      (int dlc, const uint8_t* data) { return true; }
  bool HandleSetTorque        (int dlc, const uint8_t* data) { return true; }
  bool HandleSetVelocity      (int dlc, const uint8_t* data) { return true; }
  bool HandleSetPosition      (int dlc, const uint8_t* data) { return true; }
  bool HandleCalibStart       (int dlc, const uint8_t* data) { return true; }
  bool HandleCalibAbort       (int dlc, const uint8_t* data) { return true; }
  bool HandleAnticoggingStart (int dlc, const uint8_t* data) { return true; }
  bool HandleAnticoggingAbort (int dlc, const uint8_t* data) { return true; }
  bool HandleSetHome          (int dlc, const uint8_t* data) { return true; }
  bool HandleErrorReset       (int dlc, const uint8_t* data) { return true; }
  bool HandleGetStatusword    (int dlc, const uint8_t* data) { return true; }
  bool HandleGetValue1        (int dlc, const uint8_t* data) { return true; }
  bool HandleGetValue2        (int dlc, const uint8_t* data) { return true; }
  bool HandleSetConfig        (int dlc, const uint8_t* data) { return true; }
  bool HandleGetConfig        (int dlc, const uint8_t* data) { return true; }
  bool HandleSaveAllConfig    (int dlc, const uint8_t* data) { return true; }
  bool HandleResetAllConfig   (int dlc, const uint8_t* data) { return true; }
  bool HandleSync             (int dlc, const uint8_t* data) { return true; }
  bool HandleHeartbeat        (int dlc, const uint8_t* data) { return true; }
  bool HandleStartAuto        (int dlc, const uint8_t* data) { return true; }
  bool HandleGetFwVersion     (int dlc, const uint8_t* data) { return true; }
  bool HandleDfuStart         (int dlc, const uint8_t* data) { return true; }
  bool HandleDfuData          (int dlc, const uint8_t* data) {
    if (dlc < 1 || dlc > 8) { return false; }
    return true;
  }
  bool HandleDfuEnd           (int dlc, const uint8_t* data) { return true; }
