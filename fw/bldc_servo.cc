...
class BldcServo::Impl {
 ...
 private:
  ...
  void ISR_DoSense() __attribute__((always_inline)) MOTEUS_CCM_ATTRIBUTE {
    ...
    // Configure ADC5 for software trigger and start conversion for secondary sampling
    ConfigureAdcForSoftwareTrigger(ADC5);
    ...
    // Restore ADC5 to LPTIM1 trigger mode for next cycle
    ConfigureAdcForLptim1Trigger(ADC5);
